...
const countryCodePhoneBook = phoneBook.map(function(entry) {
    let key = Object.keys(entry);
    let newEntry = Object.assign({}, entry); //to not alter original phonebook
    newEntry[key] = entry[key][0] + "-" + entry[key];
    return newEntry;
});

console.log(countryCodePhoneBook);